 ...
 */
export const isTextMessage = (content: string): boolean => {
 ...
 * Priority: Tags first (what really defines the type), then content analysis
 */
export const getMediaMessageType = (content: string): 'audio' | 'transcription' | 'audio-error' | 'video' | 'document' | 'image' | 'text' => {
  // 1. PRIMEIRA PRIORIDADE: Verificar as tags (o que realmente define o tipo)
  if (isAudioMessage(content)) return 'audio'; // <audio>
  if (isVideoMessage(content)) return 'video'; // <video>
  if (isDocumentMessage(content)) return 'document'; // <document>
  if (isTextMessage(content)) return 'text'; // <text>
  
  // 2. SEGUNDA PRIORIDADE: Verificar padrões de conteúdo (sem tags)
